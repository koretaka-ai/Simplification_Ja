...
if [ $BPE_TOKENS == "BART" ]; then
    ...
    mkdir -p $preprocessed_dir
    fairseq-preprocess --source-lang $src_lang --target-lang $tgt_lang \
        --trainpref ${DATASETS_DIR}/SNOW/tok/train --validpref ${DATASETS_DIR}/SNOW/tok/valid \
        --destdir $preprocessed_dir \
        --workers 60 \
        --joined-dictionary
else
    ...
    mkdir -p $preprocessed_dir
    fairseq-preprocess --source-lang $src_lang --target-lang $tgt_lang \
        --trainpref ${DATASETS_DIR}/SNOW/tok/bpe${BPE_TOKENS}/train --validpref ${DATASETS_DIR}/SNOW/tok/bpe${BPE_TOKENS}/valid \
        --destdir $preprocessed_dir \
        --workers 60 \
        --joined-dictionary
fi